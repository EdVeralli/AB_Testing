...
SELECT distinct 
  ...
  month
FROM 
  "caba-piba-consume-zone-db".boti_user_vars_metrics a
WHERE 
  session_creation_time 
  between cast('2024-01-08 13:05:00' as timestamp) and cast('2024-01-15 03:00:00' as timestamp) and
  vars_name='randomia' and 
  substr(session_id, 1,20) not in ('GE6NEV1ZHNDRXIAAQVGV', 'LU0VWVFLOK130EGKD1LR', 'S2311UDJQVQZ1B4L1LM2', 'D7GIKRV4B6Y7O83KBVXV', 'VNDVGYFJHMVU1GV4LUB3', 'XYRVASSYPE16OBJ8V6DZ', '2EM2NMQYWOPYN0CV8HCD',
...
canales as (
  ...
  FROM 
    todos t
  left join boti_session_metrics c on t.session_id=c.session_id
),
user_msg as(
  select * 
  ...
  ( select 
      session_id
    FROM 
      boti_message_metrics
    WHERE 
      session_creation_time between cast('2024-01-08 13:05:00' as timestamp) and cast('2024-01-15 03:00:00' as timestamp) and msg_from='user')
), 
insearch as (
  SELECT
    ins.message_id,
  ...
  WHERE 
    ts between cast('2024-01-08 13:05:00' as timestamp) and cast('2024-01-15 03:00:00' as timestamp)
...
select distinct 
  ...
  ins.max_score,
  um.vars_value
FROM boti_message_metrics  mm
LEFT JOIN insearch ins on ins.message_id = mm.id
LEFT JOIN user_msg um on um.session_id = mm.session_id
WHERE 
  mm.session_creation_time between cast('2024-01-08 13:05:00' as timestamp) and cast('2024-01-15 03:00:00' as timestamp) and substr(mm.session_id, 1,20) in 
  ( select 
    ...
    where
      ...
      (cast(vars_value as int) between 1 and 200 and um.session_creation_time between cast('2024-01-08 13:05:00' as timestamp) and cast('2024-01-15 03:00:00' as timestamp))
  )
  order by session_id, creation_time, vars_value
-- FALTAN ALGUNAS HORAS QUE EL 16 VAMOS A CORREGIR LA QUERY PARA QUE CIERRE EL 15 COMPLETO